...
export default function FormComponent() {
  ...
  const updateQueryHistory = (value: Value) => {
    checkForLocalHistory(true, value);
  }
  ...
  const checkForLocalHistory = (update: boolean, value?: Value): void => {
    ...
    if (localHistory) {
      ...
      value && localStorage.setItem('queryHistory', JSON.stringify([value]));
    }
  }

  useEffect(() => {
    checkForLocalHistory(false);
  }, []);

  return (
    <div>
      <Box bg="whitesmoke" w={{ base: "", md: "50%" }} m={{ base: "50px 10px", md: "50px auto" }} py={4} px={2} color="black" borderRadius=".5em">
